...
void KNX_INIT(void)
{
  ...
  if( strlen(SettingsText(SET_KNX_HOST)) ) {
    KNX_ip = String(SettingsText(SET_KNX_HOST));
    knx.udpAddress_set(KNX_ip.c_str());
  }
...
const char HTTP_FORM_KNX[] PROGMEM =
  ...
  "<br><center>"
  "<b>Gateway IP </b>"
  "<input style='width:52%%;' name='ip' maxlength='15' value='%s'><br>"
...
void HandleKNXConfiguration(void)
{
  ...
  else
  {
    ...
    KNX_physs_addr.value = Settings->knx_physsical_addr;
    WSContentSend_P(HTTP_FORM_KNX, KNX_ip.c_str(), KNX_physs_addr.pa.area, KNX_physs_addr.pa.line, KNX_physs_addr.pa.member);
...
void KNX_Save_Settings(void)
{
  ...
  stmp = Webserver->arg("ip");
  KNX_ip = stmp;
  SettingsUpdateText(SET_KNX_HOST, KNX_ip.c_str());
  knx.udpAddress_set(KNX_ip.c_str());


  Settings->knx_physsical_addr = KNX_addr.value;
  knx.physical_address_set( KNX_addr ); // Set Physical KNX Address of the device
  AddLog(LOG_LEVEL_DEBUG, PSTR(D_LOG_KNX D_KNX_PHYSICAL_ADDRESS ": %s %d.%d.%d"),
   KNX_ip.c_str(), KNX_addr.pa.area, KNX_addr.pa.line, KNX_addr.pa.member );
